  ...
  fetchStoreBySlug: async (slug) => {
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        // Fallback: search for first store if slug is generic or missing
        const firstQ = query(storesRef, limit(1));
        const firstSnap = await getDocs(firstQ);
        if (!firstSnap.empty) {
          const docSnap = firstSnap.docs[0];
          set({ currentStore: { id: docSnap.id, ...docSnap.data() } as Store, loading: false });
        } else {
          set({ error: 'No stores available', loading: false });
        }
      }
    } catch (err: any) {
      set({ error: err.message, loading: false });
